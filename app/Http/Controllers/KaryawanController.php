		...
		switch ($task) {
			default:
				...
				if ($validator->passes()) {
					...
					// hitung usia
					//$date1 = $request->tgl_lahir;
					$d1 = new DateTime($request->tgl_lahir);
					$d2 = new DateTime(date("Y-m-d"));
					$d3 = $d2->diff($d1);
					$usia = $d3->y;
					$update_usia = DB::table('tb_karyawan')->where('id', $id)->update(array('usia' => $usia));
	...
	public static function display()
	{
		...
		if ($mode == 'view') {
			...
			if ($row) {
				...
				$data['id'] = $id;
				return view('karyawan.public.view', $data);
			}
		} else {
			...
			return view('karyawan.public.index', $data);
		}
	}
