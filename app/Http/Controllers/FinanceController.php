...
class FinanceController extends Controller {
		...
		switch ($task)
		{
			default:
				...
				if ($validator->passes()) 
				{
					...
					if(isset($cek)){
						// update user
						DB::table('tb_users')->where('username',$request->username)->update($user);
					}else{
	...
	public function destroy( $request)
	{
		...
		{
			foreach($request->input('ids') as $r){
				$cek = DB::table('tb_finance')->select('username')->where('id',$r)->pluck('username')->first();
				$delete = DB::table('tb_users')->where('username',$cek)->delete();
			}
	...
	public static function display(  )
	{
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('finance.public.view',$data);			
			}			
		} 
		else {
			...
			return view('finance.public.index',$data);	
		}
